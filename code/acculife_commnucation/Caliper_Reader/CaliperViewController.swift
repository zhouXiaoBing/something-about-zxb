...
class CaliperViewController: UIViewController, CBCentralManagerDelegate, CBPeripheralDelegate, UITextFieldDelegate, UITableViewDataSource, UITableViewDelegate, AVSpeechSynthesizerDelegate {
...
//        cell.backgroundColor = UIColor (colorLiteralRed: 0, green: 0, blue: 0, alpha: 0)
    ...
    @IBAction func ConnectToBle(_ sender: UIButton) {
        ...
        if viewScanBle.isHidden == true {
            ...
            bleFindPeripheral.removeAll()//移除其他不连接的设备
            bleDeviceNameList.removeAll()//移除设备名列表
            bleDeviceUUIDList.removeAll()//移除UUID列表
            bleDeviceRSSIList.removeAll()//移除RSSI(信号强度指示器)列表
        ...
        if btnBleConnect.titleLabel?.text == "Disconnect" {
            if self.connectPeripheral != nil {
                centralManager.cancelPeripheralConnection(connectPeripheral)//取消次要蓝牙连接
            }
        }
        
        
        
        let transition = CATransition()//过度动画
...
//    @IBAction func ReadFromBle(_ sender: UIButton) {
    ...
    func textFieldDidBeginEditing(_ textField: UITextField) {//有报错
